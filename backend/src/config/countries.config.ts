...
export const validateIdNumber = (idNumber: string, idType: string, countryCode: string): boolean => {
  const idTypeConfig = getIdTypesForCountry(countryCode).find((t) => t.code === idType);
  if (!idTypeConfig) return false;
  const cleanId = idNumber.replace(/[\s.-]/g, '').toUpperCase();
  return new RegExp(idTypeConfig.pattern, "i").test(cleanId);
};
